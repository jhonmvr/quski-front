...
export class AutorizacionService {
  ...
  public getTokenApi(dataParam): Observable<any> {
    if (environment.production == true) {
      ...
      let token = atob(dataParam[environment.prefix + "RE020"]).replace(
        keyUnencrypt,
        ""
      );
      localStorage.setItem(environment.token_type, "basic");
      ...
      let wp = {};
      return this.http.post<BaseWrapper>(
        atob(environment.api_t),
        wp,
        optionsLoc
      );
    } else {
      const fixed = {
        token_type: environment.token_type,
        access_token: environment.apitoken,
      };
      return of(fixed);
    }
  }
  ...
  public getRelative(token: string): Observable<any> {
    ...
    const params = new HttpParams().set("prefix", environment.prefix);
    let optionsLoc = {
      headers: headersLoc,
      params: params,
    };
  ...
  public serverLogin(authData): Observable<UsuarioAuth> {
    return this.getToken().pipe(
      switchMap((tokenaouth) =>
        this.login(authData, tokenaouth).pipe(
          switchMap((usuariowp) =>
            this.getRelative(usuariowp.token).pipe(
              switchMap((dataParam) =>
                this.getTokenApi(dataParam).pipe(
                  switchMap((tokenApi) =>
                    this.getCatalogoRol(tokenApi).pipe(
                      switchMap((catalogoRol) =>
                        this.getCatalogoDivisionPolitica(tokenApi).pipe(
                          switchMap((catalogoDivision) =>
                            this.userReturn(
                              usuariowp,
                              dataParam,
                              null,
  ...
  private userReturn(
    dataLogin,
    dataParam,
    dataRoles: Array<RolWrapper>,
    credential,
    catalogoRol,
    catalogoDivision
  ): Observable<UsuarioAuth> {
    ...
    if (dataLogin && dataLogin.roles) {
      ...
      /* console.log(
        "++>termino busqueda de usuario canal: " + JSON.stringify(dataLogin)
      ); */
      ...
      x.existLogin = true;
      x.tieneDobleFactor = dataLogin.mfaRegistrado;
      ...
      if (dataLogin.agencia) {
        ...
        localStorage.setItem(
          "idResidenciaAgencia",
          dataLogin.agencia.idResidencia
        );
        localStorage.setItem(
          "idTevcolAgencia",
          dataLogin.agencia.idUbicacionTevcol
        );
        let m = catalogoDivision.catalogo.find(
          (x) => x.id == dataLogin.agencia.idResidencia
        );
        localStorage.setItem("ciudadAgencia", m ? m.nombre : "");
      }
      if (catalogoRol.catalogo) {
        const x = catalogoRol.catalogo.find(
          (p) => p.codigo == dataLogin.roles[0]
        );
      ...
      if (!dataLogin || !dataLogin.entidad) {
        y.password =
          "ERROR EN LOGIN, USUARIO O CLAVE INCORRECTO, O EL USUARIO ESTA INACTIVO";
        return of(y);
      } else {
        y.password =
          "ERROR EN LOGIN, ERROR DESCONOCIDO CONSULTE A SU ADMINISTRADOR";
        return of(y);
      }
    }
  }
  ...
  public getUserQr(codigoUsuario: string): Observable<any> {
    let params = new HttpParams();
    params = params.set("codigoUsuario", codigoUsuario);
    let headers = new HttpHeaders({ "Content-Type": "application/json" });
    let options = { headers: headers, params: params };
    //console.log("$$$$$$$$$$> url ", atob(environment.seg_p), codigoUsuario);
    return this.http.get(atob(environment.seg_p), options);
  }

  public registerQr(codigoUsuario: string): Observable<any> {
    let params = new HttpParams();
    params = params.set("codigoUsuario", codigoUsuario);
    let headers = new HttpHeaders({ "Content-Type": "application/json" });
    let options = { headers: headers, params: params };
    //console.log("$$$$$$$$$$> url ", atob(environment.seg_udf), codigoUsuario);
    return this.http.get(atob(environment.seg_udf), options);
  }

  public validateQr(codigoUsuario: string, otp: string) {
    const payload = {
    ...
    console.log(
      "$$$$$$$$$$> url vaolicacion ",
      atob(environment.seg_vdf),
      codigoUsuario,
      otp
    );
    return this.http.post(atob(environment.seg_vdf), payload, options);
  }
  ...
  public athorizate(aplicacion): Observable<Array<any>> {
    ...
    let wp = {
      codigosRol: [localStorage.getItem(environment.rolKey)],
      codigoAplicacion: environment.appkey,
    };
    return this.http
      .post(atob(environment.seg_r) + "menu", wp, optionsLoc)
      .pipe(
        map(
          (response: any) => {
  ...
  public removeRe000() {
    ...
    localStorage.removeItem(environment.token_type);
    localStorage.removeItem(environment.access_token);
    localStorage.removeItem("reUser");
    localStorage.removeItem("nombre");
    localStorage.removeItem("email");
    localStorage.removeItem("telefono");
    localStorage.removeItem(environment.rolKey);
    localStorage.removeItem("idAgencia");
    localStorage.removeItem("codigoAgencia");
    localStorage.removeItem("nombreAgencia");
    localStorage.removeItem("direccionAgencia");
    localStorage.removeItem("idResidenciaAgencia");
    localStorage.removeItem("idTevcolAgencia");
    localStorage.removeItem("ciudadAgencia");
