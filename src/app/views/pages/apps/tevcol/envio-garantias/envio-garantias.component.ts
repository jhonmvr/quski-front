...
@Component({
  selector: 'kt-envio-garantias',
  templateUrl: './envio-garantias.component.html',
  styleUrls: ['./envio-garantias.component.scss']
})
export class EnvioGarantiasComponent implements OnInit {
  catProceso:   Array<any>;
  cargardatos = new BehaviorSubject<boolean>(false);
  displayedColumns = [ 'numeroOperacion','numeroOperacionMadre','numeroFundaActual','numeroFundaMadre','agencia', 'estadoGarantia', 'ubicacionGarantia', 'nombreCliente', 'identificacionCliente'];
  ...
  estadoUbicacion = new FormControl('', [Validators.required]);
  catEstadoUbicacion = [{nombre:'UTIAGE',proceso:'UTI', ubicacion:'AGE'}, {nombre:'NECCUS',proceso:'NEC', ubicacion:'CUS'}];
  ...
  enviar(){
    ...
    if(confirm("Esta seguro que decea enviar a tevcol " + this.totalResults + " garantias de la agencia: " + this.nombreAgencia(this.w.idAgencia))){
      this.dev.enviarTevcol(this.operaciones.map(p=>{ return {codigoOperacion: p.numeroOperacion, cedulaCliente: p.identificacion}})).subscribe(t=>{
  ...
  buscarBoton(pageIndex?, pageSize?){
    ...
    this.w.idAgencia = this.agencia.value ? this.agencia.value.id : null;
    this.w.codigoEstadoProceso= this.estadoUbicacion.value.proceso;
    this.w.codigoEstadoUbicacion= this.estadoUbicacion.value.ubicacion;
